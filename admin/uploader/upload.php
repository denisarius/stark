...
//	$save_path = getcwd() . "/uploads/";				// The path were we will save the file (getcwd() may not be reliable and should be tested in your environment)
	$save_path="$_admin_uploader_path/";
	$upload_name = "Filedata";
	$max_file_size_in_bytes = 2147483647;				// 2GB in bytes
	$extension_whitelist = array("jpg", "gif", "png", 'doc', 'docx', 'xls', 'xslx', 'odt', 'rtf', 'txt', 'pdf');	// Allowed file extensions
	$valid_chars_regex = 'à-ÿÀ-ß.A-Z0-9_ !@#$%^&()+={}\[\]\',~`-';				// Characters allowed in the file name (in a Regular Expression format)
...
//	$file_name = preg_replace('/[^'.$valid_chars_regex.']|\.+$/i', "", basename($_FILES[$upload_name]['name']));
	$file_name = $_POST['upload_file_real_name'];
//	HandleError("[$file_name]\n");
